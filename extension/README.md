...
Version **0.4.8** has been uploaded to the Chrome Web Store; its review/publication state remains to be recorded. Version **0.4.9** is the next local candidate and adds atomic sign-in handoff, bounded network requests, session-race protection, clearer recovery after an expired or consumed handoff, and a direct sign-in action on the Qira orb when signed out. During a pending handoff the orb reopens the dashboard; after sign-in it opens the Capture/Dashboard actions. On the trusted Radar page both versions can open a user-selected set of up to five HTTP(S) sources as ordered background tabs. Older or unavailable extensions fall back to one browser-open attempt plus direct links for every remaining source.
