-- Keep the compact card takeaway, review timestamp, and anonymous team synthesis
-- in first-class columns when the editorial runner publishes a reviewed signal.
...
begin
  ...
  on conflict (external_run_id) do update set
    status = 'running', error_message = null, finished_at = null, updated_at = now()
  ...
  loop
    ...
    on conflict (period_type, period_key) do update set
      lede = excluded.lede, bullets = excluded.bullets,
  ...
  update public.news_items set
    editorial_run_id = null, editorial_lease_owner = null,
    editorial_lease_expires_at = null
  where editorial_run_id = run_uuid and editorial_status = 'pending';

  update public.editorial_job_runs set
    status = 'completed', processed_count = news_count,
    readout_count = synced_readout_count, finished_at = now(), updated_at = now()
  where id = run_uuid;

  return jsonb_build_object(
    'run_id', run_uuid, 'external_run_id', p_external_run_id,
    'upserted_news', news_count, 'upserted_readouts', synced_readout_count
  );
end;
$$;
